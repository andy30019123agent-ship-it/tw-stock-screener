...
## 上次做到哪
- 選股條件：技術面（均線糾結→黃金交叉→多頭發散、爆量突破）＋小詩 5 形態＋估值同業比＋填息＋籌碼＋**橫斷面相對強弱／產業輪動**；候選情報引擎 2026-07-25 起以「強勢雙確認」為入場門檻、權重看**成本後**超額。
- 資料：**TWSE／TPEX 免費官方端點抓全市場**（universe 約 1,966 檔、有量輸出約 925 檔；**已移除 FinMind**，僅留 finmind_fill.py 當補洞备援），千張大戶走集保 TDCC 免費週更。
- 自動化：GitHub Actions 每天台北 18:30（外部 cron-job.org 觸發 workflow_dispatch）跑管線→build→部署；每日 TG 選股快報優先讀 opportunities.json（與網站候選情報清單同一份），退路排序改**讀回測權重**、候選門檻 score≥4；workflow 失敗會推 TG 通知。
- 手機版卡片化、抓太少橘色警告。
